...
#+BEGIN_SRC emacs-lisp
  ...
  ;;                 ;;   :curl-args '("--cacert /c/Users/ppanko/Documents/Repos/cert/ca-bundle.trust.crt"))
  ;;    gptel-model "gpt-3.5-turbo")
  ;;   ;; keybinds   (global-set-key (kbd "C-c g t") 'gptel-org-set-topic)
...
#+BEGIN_SRC emacs-lisp
    ...
    ;; (defun gptel-send-request (prompt model callback)
    ;;   "Send a request to GPT with the given PROMPT and MODEL, and execute CALLBACK on the response."
    ;;   (let ((gptel-model model))  ;; Use the selected or default model
    ...
    (defun gptel-get-task-prefix (task-type)
      ...
      (let ((task-prefixes '(("Refactor" . "You are a coding assistant operating inside Emacs. Your task is to refactor the code provided to you. Ensure the refactored code is more efficient, follows best practices, and maintains the original functionality. Provide the refactored code only; no explanations or comments.\n\n")
                             ("Generate Documentation" . "You are a coding assistant operating inside Emacs. Your task is to generate detailed documentation for the code provided. Include comments, docstrings, or external documentation as appropriate. Provide the documentation only; no explanations or additional text.\n\n")
                             ("Write Tests" . "You are a coding assistant operating inside Emacs. Your task is to write unit tests or test cases for the code provided. Ensure the tests cover various scenarios and edge cases. Provide the tests only; no explanations or additional text.\n\n")
                             ("Translate Code" . "You are a coding assistant operating inside Emacs. Your task is to translate the provided code from one programming language to another. Ensure the translated code maintains the original functionality. Provide the translated code only; no explanations or additional text.\n\n")
    ...
    ;; (defun gptel-handle-insert (response insert-type)
    ;;   "Insert RESPONSE based on INSERT-TYPE. INSERT-TYPE should be 'replace, 'append, or 'prepend."
    ;;   (if (use-region-p)
    ;;       (let ((start (region-beginning))
    ;;             (end (region-end)))
    ;;         (message "Previous region starts at: %d, ends at: %d" start end)
    ;;         (save-excursion
    ;;           (cond
    ;;            ((eq insert-type "replace")
    ;;             (goto-char start)
    ;;             (delete-region start end)
    ;;             (insert response))
    ;;            ((eq insert-type "append")
    ;;             (goto-char end)
    ;;             (insert "\n" response))
    ;;            ((eq insert-type "prepend")
    ;;             (goto-char start)
    ;;             (insert response "\n")))))))
    ...
    (defun gptel-handle-insert (response insert-type)
      "Insert RESPONSE based on INSERT-TYPE. INSERT-TYPE should be 'replace, 'append, or 'prepend."
      ;; (if (use-region-p)
      (message "insert type is:" insert-type)
      (message "Previous region starts at: %d, ends at: %d" (region-beginning) (region-end))
      (save-excursion
        (cond
         ((eq insert-type "replace")
    ...
    (defun gptel-handle-response (response task-type insert-type)
      "Handle the RESPONSE from GPT based on TASK-TYPE and INSERT-TYPE."
      (message "Debug: Received response: %s" response)  ;; Debugging statement
      (if (eq insert-type 'message)
          (message "GPT response: %s" response)
        (gptel-handle-insert response insert-type)
    ...
    (defun gptel-send-request (prompt task-type insert-type)
      "Send a request to GPT with the given PROMPT, and process the response with gptel-handle-response."
      (gptel-request
          prompt
        :context (list :insert insert-type
                       :task task-type)
        :callback (lambda (response info)
                    (if (not response)
                        (message "ChatGPT response failed with: %s" (plist-get info :status))
                      ;; Log the raw response and info for debugging
                      ;; (message "Debug: Raw Response: %s" response)
                      ;; (message "Debug: Info: %s" info)
                      (let* ((context (getf info :context))  ; Access the nested context
                             (task-type (getf context :task))  ; Access first additional argument
                             (insert-type (getf context :insert)))                       ;; Attempt to parse as JSON, and pass the response to gptel-handle-response
                        ;; (condition-case err
                        ;;     (let ((json-response (if (stringp response)
                        ;;                              (json-parse-string response)
                        ;;                            response)))
                        ;;       (message "Debug: JSON Response Parsed Successfully: %s" json-response)
                        ;;       (gptel-handle-response json-response task-type insert-type))  ;; Call gptel-handle-response
                        ;;   (json-error
                        ;;    (message "Error parsing JSON: %s" (error-message-string err))
                        ;;    ;; Call gptel-handle-response with the raw response
                        (gptel-handle-response response task-type insert-type))))))

    (defun gptel-handle-task (task-type insert-type)
      "Handle sending the last marked region or current line to GPT based on the TASK-TYPE and INSERT-TYPE.
                     INSERT-TYPE should be 'replace, 'append, 'prepend, or 'message."
      (let* ((selected-code (gptel-get-code task-type))
             (prompt (gptel-prepare-prompt task-type selected-code))
             (model (gptel-select-model)))
        (if selected-code
            (gptel-send-request prompt task-type insert-type)  ;; No need for a callback here
          (message "No region or line selected."))))


    ;; (defun gptel-send-request (prompt callback)
    ;;    "Send a request to GPT with the given PROMPT and execute CALLBACK on the response."
    ;;    (gptel-request
    ;;        prompt
    ;;      :callback (lambda (response info)
    ;;                  (if (not response)
    ...
    ;;                    (message "Debug: Raw Response: %s" response)
    ;;                    (message "Debug: Info: %s" info)
    ;;                    ;; Attempt to parse as JSON, and pass the response to the callback
    ;;                    (condition-case err
    ;;                        (let ((json-response (if (stringp response)
    ;;                                                 (json-parse-string response)
    ;;                                               response)))
    ;;                          (message "Debug: JSON Response Parsed Successfully: %s" json-response)
    ;;                          (funcall callback json-response))  ;; Call the passed callback with the parsed response
    ;;                      (json-error
    ;;                       (message "Error parsing JSON: %s" (error-message-string err))
    ;;                       ;; Call the callback with the raw response even if parsing fails
    ...
    ;;   (let* ((selected-code (gptel-get-code task-type))
    ;;          (prompt (gptel-prepare-prompt task-type selected-code))
    ;;          (model (gptel-select-model)))  ;; Use model if needed in future expansions
    ;;     (if selected-code
    ;;         (gptel-send-request
    ;;          prompt
    ;;          (lambda (response)
    ;;            (gptel-handle-response response task-type insert-type))) ;; Pass gptel-handle-response as callback
    ;;       (message "No region or line selected."))))
    ...
    ;; (defun test-gptel-send-request ()
    ;;   "Function to test `gptel-send-request` and print the response."
    ;;   (interactive)
    ;;   (gptel-send-request "Hello, GPT!" #'my-callback))

    ;; (defun gptel-send-request (prompt callback)
    ;;   "Send a request to GPT with the given PROMPT and execute CALLBACK on the response."
    ;;   (gptel-request prompt
    ;;     :callback (lambda (response proc-info)
    ;;                 (let ((http-msg (plist-get proc-info :status))
    ;;                       (error (plist-get proc-info :error)))
    ;;                   (message "Debug: HTTP message: %s, Error: %s, Response: %s" http-msg error response)  ;; Debugging statement
    ;;                   (if (or error (not (stringp response)))
    ;;                       (message "Error: Invalid response received from GPT: %s" (or error http-msg))
    ;;                     (funcall callback response))))))
...
gptel copilot functions
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    ;; (defun gptel-curl--parse-response-advice (orig-fun &rest args)
    ;;   "Advice to add detailed logging to `gptel-curl--parse-response`."
    ;;   (condition-case err
    ;;       (let ((response (apply orig-fun args)))
    ;;         (message "Debug: Parsed Response: %s" response)
    ;;         response)
    ;;     (error
    ;;      (message "Error in `gptel-curl--parse-response`: %s" (error-message-string err))
    ;;      (message "%s" (with-output-to-string (backtrace)))
    ;;      (signal (car err) (cdr err)))))

    ;; (advice-add 'gptel-curl--parse-response :around #'gptel-curl--parse-response-advice)
...
OLD: Always make R buffer split right & R code buffer split left, as shown [[https://lambdaland.org/posts/2022-12-27_repl_buffer_on_the_right/][here]] now
