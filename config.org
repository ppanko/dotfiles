...
#+BEGIN_SRC emacs-lisp
    (defun create-r-project-dir-structure (name)
      (interactive "sName: ")
      (make-directory name)
      (setq pastDir default-directory)
      (cd name)
      (make-directory "R")
      (make-directory "data")
...
#+BEGIN_SRC emacs-lisp
  (defun insert-roxygenated-header (title)
    (interactive "sTitle: ")
    (setq date (shell-command-to-string "echo -n $(date +%m/%d/%Y)"))
    (insert (format
"#' ---
#' title: %s
#' author: Pavel Panko
...
#+BEGIN_SRC emacs-lisp
  (if (eq system-type 'windows-nt)
      (setq ess-directory-containing-R "C:/Program Files/"
	    inferior-ess-r-program-name "C:/Program Files/R/R-3.6.3/bin/x64/Rterm.exe"))
#+END_SRC
** Flycheck
