...
#+BEGIN_SRC emacs-lisp
  (when (eq system-type 'windows-nt)
    ...
    ;; Buffer-local fixes when shell buffers are created
    (add-hook 'shell-mode-hook
            (lambda ()
              ;; Make the strip filter buffer-local
              (add-hook 'comint-output-filter-functions
                        #'comint-strip-ctrl-m nil t)

              ;; After the subprocess is actually created, set its coding.
              ;; comint-exec-hook can receive args, so accept them but ignore.
