    ...
    (let ((sym (ess-symbol-at-point)))
      (if sym
          (ess-send-string (ess-get-process ess-local-process-name)
                           (concat "DT::datatable(" (symbol-name sym) ")\n") t)
        (message "No valid R symbol at point"))))

;;  (define-key ess-r-mode-map (kbd "C-c v") 'r-view-df-at-point)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun set-projectile-wd-on-startup ()
...
#+BEGIN_SRC emacs-lisp
  (use-package ess-r-mode
    :ensure ess
    :hook ((inferior-ess-mode . my-inferior-ess-init)
...
#+BEGIN_SRC emacs-lisp
  (when (eq system-type 'windows-nt)
      (setq-default inferior-R-program-name "C:\\Program Files\\R\\R-4.2.2\\bin\\x64\\Rterm.exe"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package projectile
      ...
      (projectile-mode +1)
      :custom
      (projectile-register-project-type 'r #'projectile-r-project-file-p
                                      :project-file #'projectile-r-project-p))
#+END_SRC
** Python
