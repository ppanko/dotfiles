...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'ess-r-mode-hook #'p3/ess-config))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
    ...
    ;;                           "Get the selected region as a string.
    ;;                                                       Return nil if no region is selected."
    ;;                           (if (use-region-p)
    ...
    ;;                       "Insert RESPONSE based on INSERT-TYPE.
    ;;                     INSERT-TYPE should be 'replace, 'append, or 'prepend."
    ;;                       (let ((text (json-serialize response)))
    ;;                         (save-excursion
    ;;                           (cond
    ...
    ;;                (if (not response)
    ;;                    (message "Error: Received nil response")
    ;;                  ;; Check if the response is a string and attempt to parse as JSON
    ;;                  (condition-case err
    ...
    ;;       (message "GPT response: %s" (json-serialize response))
    ...
    ;;     (if selected-code
    ;;         (gptel-send-request prompt model
    ;;                             (lambda (response)
