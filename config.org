...
Platform-specific discovery and process configuration live in
=lisp/p3-platform.el=. On Windows this selects the newest usable Rtools/MSYS2
environment and R installation unless overridden in =secrets.el=, then exposes
the resulting shell, Git/Unix-tool, and Hunspell paths before other packages
need them.
...
Platform-specific Windows shell setup is handled by =p3-platform= near startup.
Project-aware vterm behavior remains in =lisp/p3-terminal.el=; package wiring
stays here.
