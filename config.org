...
#+BEGIN_SRC emacs-lisp
  (defun p3/add-pipe-and-step ()
    "Insert magrittr pipe quickly without triggering ESS reindent."
    (interactive)
    (just-one-space 1)
    (insert "%>%\n"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
    (defconst p3/targets-r-contents
      (concat
       "##
  pkgs <- installLoadPackages(
    ## Environment management
    \"renv\", \"targets\", \"tarchetypes\", \"here\", \"pkgload\", \"devtools\",
    ## Data I/O
    \"readr\", \"data.table\", \"tidypolars\", \"arrow\", \"writexl\",
    ## Data manipulation
    \"dplyr\", \"lubridate\", \"tidyr\", \"purrr\", ##\"stringr\",
    ## Visualization
    \"DT\", ## \"ggplot2\"
    ## parallel processing
    \"crew\", \"crew.cluster\"
  ...
  LaTeX: pdfLaTeX"))
    ...
    (defconst p3/gitignore-contents
      (concat
       "Archive/
        .Rhistory
        ,*~
    ...
    ;; Default R scripts and their titles
    (defconst p3/r-script-titles
      '(("01_prepareData.R"    . "Preprocess data")
        ("02_computeResults.R" . "Compute results")
        ("utils.R"             . "Helper functions")))

    (defconst p3/base-directories
      '("R" "data" "data-raw" "output" "reports" "bin" "reports/graphics"))

    (defconst p3/utils-r-contents
      (concat
       "##
    ...
    (defun p3/generate-roxygen-header (title &optional minimal)
      "Return a roxygen/knitr header for an R script with TITLE.
              If MINIMAL is non-nil, only include the YAML block (title/author/date),
              without headings or setup chunk."
    ...
    (defun p3/--make-dirs (dirs)
      "Create each directory listed in DIRS if it does not already exist."
      (dolist (dir dirs)
        (unless (file-exists-p dir)
          (make-directory dir t))))

    (defun p3/--write-file (path contents project-root)
      "Write CONTENTS to PATH using a temp buffer."
      (with-temp-file (expand-file-name path project-root)
        (insert contents)))


    (defun p3/create-default-r-scripts (project-root)
      "Create default R scripts with headers inside PROJECT-ROOT/R."
      (let ((r-dir (expand-file-name "R" project-root)))
        (unless (file-exists-p r-dir)
          (make-directory r-dir t))
        (dolist (pair p3/r-script-titles)
          (let* ((file (car pair))
                 (title (cdr pair))
                 (path (expand-file-name file r-dir))
                 (minimal (string-equal file "utils.R")))
            (with-temp-file path
              (insert (p3/generate-roxygen-header title minimal))
              (when (string-equal file "utils.R")
                (insert p3/utils-r-contents)))))))

    (defun p3/create-targets-file ()
      "Create a _targets.R file in the project root with a full header."
      (let ((file "_targets.R")
            (title "_targets pipeline")
            (path (expand-file-name "_targets.R")))
        (with-temp-file path
          (insert (p3/generate-roxygen-header title))
          (insert p3/targets-r-contents))))


    (defun p3/create-r-project-dir-structure (&optional include-targets)
      "Create an R project named NAME with standard directories, R scripts, and optional targets directories."
      (interactive "p")

      (let* ((path (read-file-name "Create R project at: " (expand-file-name "~")))
         (project-root (directory-file-name path))
         (project-name (file-name-nondirectory project-root))
         (parent-dir (file-name-directory project-root))
         (rproj-file (concat project-name ".Rproj")))

        (progn
          ;; Create root
          (unless (file-exists-p project-root)
            (make-directory project-root))

          (let ((default-directory project-root))

            ;; Base directories
            (p3/--make-dirs p3/base-directories)

            ;; Create top-level _targets.R
            (p3/create-targets-file))
          ;; Default R scripts
          (p3/create-default-r-scripts project-root)
          ;; Project files
          (p3/--write-file rproj-file p3/rproj-contents project-root)
          (p3/--write-file ".projectile" "" project-root)
          (p3/--write-file ".gitignore" p3/gitignore-contents project-root))
...
#+BEGIN_SRC emacs-lisp
  (use-package company
    ...
    :config
    (setq r-company-backends '((:separate
             ;; help
    ...
    (defun p3/ess-company-config ()
      ...
      (setq-local company-backends r-company-backends)))
  ;; for debugging
...
#+BEGIN_SRC emacs-lisp
  (defun p3/use-project-root-as-default-dir ()
      "Set buffer-local `default-directory` to the Projectile project root if available."
...
Generic functions for running code in the local R processor

#+BEGIN_SRC emacs-lisp
  (defun p3/ess-run-at-point (template)
    "Run R code in ESS using TEMPLATE, substituting the symbol at point.
  TEMPLATE should be a string containing `%s` where the symbol name should go."
    (interactive "sR code template (use %s for symbol): ")
    (let ((sym (ess-symbol-at-point)))
      (if sym
          (ess-send-string
           (ess-get-process ess-local-process-name)
           (format template (symbol-name sym))
           t)
        (message "No valid symbol at point."))))

  (defun p3/ess-run (code)
    "Run arbitrary R CODE string in the current ESS process."
    (interactive "sR code: ")
    (ess-send-string
     (ess-get-process ess-local-process-name)
     (concat code "\n")
...
#+BEGIN_SRC emacs-lisp
  (defun p3/tar-make ()
    ...
    (interactive)
    (inferior-ess-r-force)
    (p3/ess-run "targets::tar_make()"))

  (defun p3/tar-load-at-point ()
    "Run targets::tar_load(<symbol-at-point>)"
    (interactive)
    (inferior-ess-r-force)
    (p3/ess-run-at-point "targets::tar_load(%s)"))

  (defun p3/tar-make-debug ()
    "Run targets::tar_make(debug_options)"
    (interactive)
    (inferior-ess-r-force)
    (p3/ess-run "targets::tar_make(callr_function = NULL, use_crew = FALSE, as_job = FALSE)"))

#+END_SRC

DT viewer function to load in new R session

#+BEGIN_SRC emacs-lisp
  (defun p3/load-view-df-function ()
    "Load the view_df() R helper into the current ESS R session."
    (ess-remote-eval-string
     ...
     "view_df <- function(
        ...
        name     = deparse(substitute(x)),
        file     = 'dt_viewer.html'##,
        ## template = 'dt_title_template.html'
      ) {
          widget <- DT::datatable(
          x,
          options = list(autoWidth = TRUE),
...
#+BEGIN_SRC emacs-lisp
  (use-package ess-r-mode
    ...
    :hook ((inferior-ess-mode . p3/my-inferior-ess-init)
           (ess-r-post-run-hook . p3/set-projectile-wd-on-startup)
           (ess-r-mode-hook . p3/load-view-df-function)
           (ess-r-mode-hook . p3/ess-company-config)
           (ess-r-mode-hook . p3/use-project-root-as-default-dir)
    ...
    :bind (:map ess-mode-map
                ...
                ;; Pipe operatormultiMdsDf
                ("C-." . p3/add-pipe-and-step)
                ...
                ("C-c m" . p3/tar-make)
                ("C-c d" . p3/tar-make-debug)
                ("C-c l" . p3/tar-load-at-point))
...
#+BEGIN_SRC emacs-lisp
  (use-package elpy
    ...
    (if (eq system-type 'windows-nt)
        (setq python-shell-interpreter "C:/Users/ppanko/AppData/Local/Programs/Python/Python312/python.exe"))
    (if (eq system-type 'gnu/linux)
...
#+BEGIN_SRC emacs-lisp
  ...
  (advice-add #'ess-force-buffer-current :before #'p3/ess-ensure-project-process)
#+END_SRC
