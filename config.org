...
Update packages explicitly, so starting Emacs never changes the installed
package set unexpectedly. Run =M-x p3/package-update= when ready to review and
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun p3/package-update ()
    "Refresh package metadata and interactively upgrade installed packages."
    (interactive)
...
Use the regular shell on Windows and a project-aware vterm interface on Linux.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defcustom p3/vterm-window-height 0.35
    "Height of the project terminal side window."
    :type 'number
    :group 'p3/terminal)
  ...
  (defun p3/vterm-display (buffer)
    "Display BUFFER in the terminal side window and select it."
    (let ((window
           (display-buffer
            buffer
            `((display-buffer-reuse-window
              ...
              (side . bottom)
              (slot . 0)
              (window-height . ,p3/vterm-window-height)))))
      (unless (window-live-p window)
        (user-error "Unable to display terminal buffer"))
      (select-window window)
      (goto-char (point-max))
      buffer))

  (defun p3/vterm-toggle (&optional new-session)
    "Toggle the project terminal; with a prefix, create a NEW-SESSION."
    (interactive "P")
    (let* ((buffer (cond
                    (new-session (p3/vterm-buffer t))
                    ((derived-mode-p 'vterm-mode) (current-buffer))
                    (t (p3/vterm-buffer))))
           (window (get-buffer-window buffer (selected-frame))))
      (if window
          (quit-window nil window)
        (p3/vterm-display buffer))))

  (defun p3/vterm-new ()
    "Create and display another terminal for the current project."
    (interactive)
    (p3/vterm-display (p3/vterm-buffer t)))
  ...
  (defun p3/vterm-switch ()
    "Select and display an existing terminal session."
    (interactive)
    (p3/vterm-display (p3/vterm-read-buffer)))

  (defun p3/vterm-open-here (&optional new-session)
    "Open the project terminal in the current window.
With a prefix argument, create a NEW-SESSION."
    (interactive "P")
    (pop-to-buffer-same-window (p3/vterm-buffer new-session)))
