...
Create a variable that defines the linux subsystem path and make sure emacs looks for utils there first

#+BEGIN_SRC emacs-lisp
  (when (eq system-type 'windows-nt)
    (defvar p3-utils-dir "C:/rtools43/usr/bin/")

    (setq exec-path (cons p3-utils-dir exec-path)))
...
#+BEGIN_SRC emacs-lisp
  (use-package gptel
    ...
    ;; (defun gptel-set-curl-args ()

    ;; (if (eq system-type 'windows-nt)
        ;; (gptel-make-openai
        ;;  :stream t
        ;;  :curl-args '("--cacert" "~/cert/ca-bundle.trust.crt")))
       ;; :models '("gpt-3.5-turbo" "gpt-3.5-turbo-16k" "gpt-4"
         ;;           "gpt-4-turbo-preview" "gpt-4-32k" "gpt-4-1106-preview"
         ;;           "gpt-4-0125-preview")


    ;;  "Set curl arguments for gptel based on the operating system."
      ;; (if (eq system-type 'windows-nt)
      ;;     (setf (gptel-backend-curl-args gptel--openai)
      ;;           '("--cacert" "~/cert/ca-bundle.trust.crt")))
      ;;)
...
#+BEGIN_SRC emacs-lisp
  (when (eq system-type 'windows-nt)
      (progn
        (setq explicit-shell-file-name (concat p3-utils-dir "zsh.exe")
