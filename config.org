* Startup

Package bootstrap and dependency recovery are handled in =init.el= before this
literate configuration is loaded. Keep source selection, compilation, secrets,
and Windows Rtools/MSYS2 discovery explicit here because their order affects
all later local modules and subprocesses.

#+BEGIN_SRC emacs-lisp
  (setq load-prefer-newer t)

  (use-package auto-compile
    :demand t
    :config
    (auto-compile-on-load-mode)
    (auto-compile-on-save-mode))
#+END_SRC

Load machine-local secrets before platform discovery so Windows Rtools and R
overrides are available when their respective setup stages run.

#+BEGIN_SRC emacs-lisp
  (defconst p3/secrets-file
    (expand-file-name "secrets.el" user-emacs-directory)
    "Private configuration file loaded outside version control.")
  (when (file-exists-p p3/secrets-file)
    (load-file p3/secrets-file))
#+END_SRC

Platform-specific discovery and defaults live in =lisp/p3-platform.el=. On
Windows, select the Rtools/MSYS2 environment here so Git and other
subprocess-using packages see its Unix tools early in startup. Configure the
platform-appropriate TRAMP default at the same boundary. R and shell
configuration remain at their subsystem boundaries below.

#+BEGIN_SRC emacs-lisp
  (eval-and-compile
    (add-to-list 'load-path (expand-file-name "lisp" user-emacs-directory)))

  (use-package p3-platform
    :ensure nil
    :demand t
    :config
    (p3/windows-configure-rtools)
    (p3/platform-configure-tramp))
#+END_SRC

* Base

Broad global configuration lives in =lisp/p3-config-base.el=: startup UI,
configuration commands, package-update wiring, process and coding defaults,
backups, Dired, and the keybinding atlas.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-base)
#+END_SRC

* Appearance

Theme, fonts, frame chrome, mode-line presentation, and icon presentation live
in =lisp/p3-config-appearance.el=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-appearance)
#+END_SRC

* Editing

Generic editing semantics and package setup live in =lisp/p3-config-editing.el=,
including completion-adjacent editing tools, diagnostics, spelling, color
previews, the simple C++ compile hook, and LaTeX editing hooks.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-editing)
#+END_SRC

* Modes
** Buffers, Windows, and Frames

Window selection, navigation, resizing, and the existing narrow ESS process
side-window rule live in =lisp/p3-config-workspace.el=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-workspace)
#+END_SRC

** Completion-related

Vertico, Orderless, Consult, Embark, Company, shared Eglot wiring, and their
existing helper configuration live in =lisp/p3-config-completion.el=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-completion)
#+END_SRC

** ESS

ESS package wiring, R-mode hooks, keybindings, buffer defaults, RMarkdown
coding/compile defaults, and ESS-specific Company configuration live in
=lisp/p3-config-ess.el=. Project-aware ESS process/session behavior remains in
=lisp/p3-ess.el=; project templates, targets commands, and other user-facing R
workflow commands remain in =lisp/p3-r-tools.el=. Managed R =languageserver=
bootstrap and Eglot integration live in =lisp/p3-r-language-server.el=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-ess)
#+END_SRC

On Windows, select the R executable here, preserving the existing subsystem
startup boundary.

#+BEGIN_SRC emacs-lisp
  (p3/windows-configure-r-program)
#+END_SRC

** Git

Git subprocess/synchronization behavior lives in =lisp/p3-git.el=; Magit,
git-gutter, and bindings live in =lisp/p3-config-git.el=. Forge is an optional
remote issue/review layer loaded after Magit; local Git and Magit workflows do
not depend on Forge being authenticated, online, or able to support a remote.

For a supported provider, configure credentials through Forge/Ghub's normal
=auth-source= setup rather than tracked configuration. For GitHub.com, follow
[[https://docs.magit.vc/devel/forge/Setup-for-Githubcom.html][Forge's GitHub setup]], including the =github.user= Git setting and its
machine-local =auth-source= credential. From a Magit buffer, =N / a= adds the
current repository to Forge's local database on first use; thereafter =N=
opens Forge's native dispatch for issues, pull requests, supported review
actions/metadata, and topic discussion/comments. Unsupported or partially
supported remotes remain ordinary Magit/Git repositories.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-git)
#+END_SRC

** GnuPG

Windows GnuPG home-path normalization lives in =lisp/p3-platform.el=. Keep the
call here so it runs at the same subsystem boundary as before.

#+BEGIN_SRC emacs-lisp
  (p3/windows-configure-gnupg)
#+END_SRC

** GPT

GPTel package/backend registration and activation live in
=lisp/p3-config-gptel.el=. The thin project-context and cut-out task layer,
including native rewrite wrappers and explicit Git-diff snapshots, lives in
=lisp/p3-gptel.el=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-gptel)
#+END_SRC

** org

Org core behavior lives in =lisp/p3-org.el=. Declarative Org, Babel, TODO,
Agenda, LaTeX export, PDF-opening, and export-activation configuration lives in
=lisp/p3-config-org.el=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-org)
#+END_SRC

** org-roam

Org-roam package configuration lives in =lisp/p3-config-org-roam.el= and
reusable Roam search, capture, filtering, and agenda helpers live in
=lisp/p3-org-roam.el=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-org-roam)
#+END_SRC

** Presentation

Org presentation package wiring lives in =lisp/p3-config-org-present.el=;
state capture/restoration and presentation commands live in
=lisp/p3-org-present.el=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-org-present)
#+END_SRC

** Project

Native project identity is established early by =lisp/p3-project.el=.
User-facing project bindings live in =lisp/p3-config-project.el= and use the
built-in =project.el= command map.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-project)
#+END_SRC

** Python

Python package wiring, mode hooks, Python-specific Eglot formatting, and
diagnostic configuration live in =lisp/p3-config-python.el=. Reusable
interpreter, language-server, and REPL behavior remains in =lisp/p3-python.el=;
generic Eglot rename/code-action bindings are owned by the shared
completion/navigation layer above.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-python)
#+END_SRC

** References

Reference capture, retrieval, citation, literature-note, project-association,
and PDF behavior lives in =lisp/p3-config-reference.el=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-reference)
#+END_SRC

** Shell

Shared project-aware Bash wiring lives in =lisp/p3-config-terminal.el=.
Project-shell session behavior lives in =lisp/p3-terminal.el=, while
platform-specific Bash discovery and Windows shell mechanics remain in
=p3-platform=.

#+BEGIN_SRC emacs-lisp
  (p3/config-load-module 'p3-config-terminal)
#+END_SRC
