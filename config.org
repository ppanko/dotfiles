...
=auth-source= setup rather than tracked configuration. For Github.com, also set
the Forge identity Git setting (for example, =git config --global github.user
USERNAME=). From a Magit buffer, =N a= adds the current repository to Forge's
local database on first use; thereafter =N= opens Forge's native dispatch for
issues, pull requests, discussion, and remote-topic operations. Unsupported or
partially supported remotes remain ordinary Magit/Git repositories.
