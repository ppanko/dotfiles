...
(defconst p3/r-tidyverse--schema-preserving-verbs
  '("filter" "arrange" "group_by" "relocate")
  "Supported upstream verbs known to preserve the input column set.")
...
(defun p3/r-tidyverse--live-columns (symbol)
  "Return live column names for simple SYMBOL without starting or blocking R.

Only the already-running ESS process associated with the current project is
consulted.  Busy, missing, stale, and errored processes return nil quietly."
...
(defun p3/r-tidyverse--apply-stage (verb columns args)
  "Return schema after supported upstream VERB over COLUMNS with ARGS."
  (cond
...
(defun p3/r-tidyverse-columns-at-point ()
  ...
  (when-let* ((call (p3/r-tidyverse--current-call))
    ...
    (let* ((function-start (plist-get call :function-start))
           (pipe (p3/r-tidyverse--pipe-before function-start)))
      (if pipe
          (p3/r-tidyverse--resolve-expression-before pipe)
