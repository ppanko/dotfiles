...
(ert-deftest p3-org-roam-project-open-reuses-project-resume-workflow ()
  (let (resumed)
    (cl-letf (((symbol-function 'p3/org-roam-project-context)
               (lambda () "hub"))
              ((symbol-function 'p3/org-roam--hub-node)
              ...
              ((symbol-function 'p3/org-roam-project-root-for-hub-id)
               (lambda (_id) "/tmp/repo/"))
              ((symbol-function 'p3/project-resume-root)
               (lambda (root) (setq resumed root))))
      (p3/org-roam-project-open)
      (should (equal resumed "/tmp/repo/")))))
