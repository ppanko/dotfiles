...
(ert-deftest p3-org-roam-project-open-routes-to-associated-root ()
  (let (switched opened)
    (cl-letf (((symbol-function 'p3/org-roam-project-context)
               (lambda () "hub"))
              ((symbol-function 'p3/org-roam--hub-node)
               (lambda (_id) 'hub-node))
              ((symbol-function 'p3/org-roam-project-root-for-hub-id)
               (lambda (_id) "/tmp/repo/"))
              ((symbol-function 'p3/project-switch-to-tab)
               (lambda (root) (setq switched root)))
              ((symbol-function 'dired)
               (lambda (root &optional _switches) (setq opened root))))
      (p3/org-roam-project-open)
      (should (equal switched "/tmp/repo/"))
      (should (equal opened "/tmp/repo/")))))

(ert-deftest p3-org-roam-project-open-errors-without-project-context ()
  (cl-letf (((symbol-function 'p3/org-roam-project-context) (lambda () nil)))
    (should-error (p3/org-roam-project-open) :type 'user-error)))

(ert-deftest p3-org-roam-project-open-errors-without-local-root ()
