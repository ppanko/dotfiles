...
(ert-deftest p3-package-accepts-built-in-package-without-descriptor ()
  (let ((package-alist nil)
        (package-activated-list nil))
    (cl-letf (((symbol-function 'package-built-in-p)
               (lambda (package &optional _min-version)
                 (eq package 'p3-builtin-fixture)))
              ((symbol-function 'package-installed-p)
               (lambda (package &optional _min-version)
                 (eq package 'p3-builtin-fixture)))
              ((symbol-function 'package-install)
               (lambda (&rest _)
                 (ert-fail "built-in package should not be installed"))))
      (should-not (p3/package--descriptors 'p3-builtin-fixture))
      (should (eq (p3/package-install-resilient 'p3-builtin-fixture)
                  'p3-builtin-fixture)))))

(ert-deftest p3-package-repairs-user-override-of-built-in-package ()
  (let* ((root (make-temp-file "p3-package-test-" t))
         (package-user-dir root)
         (package-directory-list nil)
         (package-alist nil)
        ...
        (progn
          ...
          (cl-letf (((symbol-function 'package-built-in-p)
                    ...
                    ((symbol-function 'package-install)
                     (lambda (&rest _)
                       (ert-fail
                        "repairable user override unexpectedly reached package-install"))))
            (should-not (p3/package-autoloads-healthy-p 'p3-broken))
            (should (eq (p3/package-install-resilient 'p3-broken)
                        'p3-broken)))
          (should (file-readable-p autoload-file))
